...
class ComposeTweetState extends ChangeNotifier {
  ...
  Future<String> loadData(String tweet) async {
    String output;
    String sentimentTweet;
    String newsTweet;
    String text = tweet;
    Uri url = Uri.parse('https://tomatinator-api.herokuapp.com/api?Query=' + text);
    Uri url2 = Uri.parse('https://fakenews-api-file.herokuapp.com/api?Query=' + text);
    var Data = await Getdata(url);
    var Data2 = await Getdata(url2);
    var decodedData = jsonDecode(Data)['Query'];
    var decodedData2 = jsonDecode(Data2)['Query'];
    if (decodedData == 'Positive') {
      sentimentTweet = 'Cool, your tweet seems to be positive!';
    } else if (decodedData == 'Neutral') {
      sentimentTweet = 'Your tweet seems to be neutral.';
    } else {
      sentimentTweet = 'Hmmm... Your tweet seems to be negative :(';
    }

    if (decodedData2 == 'fake') {
      newsTweet = 'Looks like the tweet contains fake news. Please fact-check from the appropriate sources! ';
    } else {
      newsTweet = 'Looks like your tweet does not contain any fake news. Awesome!';
    }
    output = sentimentTweet + '\n \n' + newsTweet;
    return output;
  }
  ...
  /// Send notificatin by using firebase notification rest api;
